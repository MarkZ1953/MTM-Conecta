...
type AttendanceBulkDeleteDialogProps = {
  ids: number[];
  open: boolean;
...
export function AttendanceBulkDeleteDialog({
  ids,
  open,
  setOpen,
  onSuccess,
}: AttendanceBulkDeleteDialogProps) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = ids.map((id) => attendanceAPI.softDelete({ id }));
      ...
      <div className="flex align-items-start gap-3">
        ...
        <div>
          ...
          <p className="m-0 text-700">
            ¿Estás seguro de que deseas eliminar permanentemente <strong>{ids.length}</strong> registro(s) seleccionado(s)?
          </p>
        </div>
      </div>
    </Dialog>
