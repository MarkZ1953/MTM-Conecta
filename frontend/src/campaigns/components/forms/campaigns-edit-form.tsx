...
const getDefaultValues = (campaign?: Campaign | null): CampaignPayload => ({
  subject: campaign?.subject ?? "",
  content_type: campaign?.content_type ?? "IMAGE",
  recipient_group: campaign?.recipient_group ?? "DONORS",
...
export const CampaignsEditForm = ({
  ...
  campaignObj,
  setRefresh,
}: CampaignsEditFormProps) => {
  const form = useForm<CampaignPayload>({
    resolver: yupResolver(campaignSchema) as any,
    defaultValues: getDefaultValues(campaignObj),
  });

  useEffect(() => {
    if (open) form.reset(getDefaultValues(campaignObj));
  }, [form, open, campaignObj]);
  ...
  const onSubmit = async (data: CampaignPayload) => {
    ...
    // En edición, el archivo es obligatorio solo si NO había uno previo
    if (data.content_type === "IMAGE" && !data.image && !campaignObj.image) {
      toast.error("Debes subir una imagen para este tipo de campaña.");
      return;
    }
    if (data.content_type === "PDF" && !data.document && !campaignObj.document) {
      toast.error("Debes subir un PDF para este tipo de campaña.");
      return;
    }

    // Solo se envían los archivos si el usuario subió uno nuevo
    const payload: Partial<CampaignPayload> = {
      ...
      image: data.content_type === "IMAGE" && data.image ? data.image : undefined,
      document: data.content_type === "PDF" && data.document ? data.document : undefined,
    };

    try {
      const { status, data: responseData } = await campaignsAPI.update({ id: campaignObj.id, data: payload });
      ...
      throw new Error(responseData ? JSON.stringify(responseData) : "No se pudo actualizar la campaña.");
    } catch (error) {
    ...
    >
      <FormProvider {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <CampaignsFormFields
            existingImage={campaignObj?.image}
            existingDocument={campaignObj?.document}
          />
        </form>
      </FormProvider>
    </Dialog>
