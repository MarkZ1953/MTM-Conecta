...
import { companyCreateSchema } from "@/cap-collection/cap-collection.schemas";
import { companiesAPI } from "@/cap-collection/cap-collection.api";
import { toast } from "@/components";
import type { CompanyPayload } from "@/cap-collection/cap-collection.types";
...
const defaultValues: CompanyPayload = {
  nit: "",
  business_name: "",
  contact_name: "",
  contact_email: "",
...
export const CompaniesCreateForm = ({
  ...
  const form = useForm<CompanyPayload>({
    resolver: yupResolver(companyCreateSchema) as any,
    defaultValues,
  });
  ...
  const onSubmit = async (data: CompanyPayload) => {
    try {
      const { status, data: responseData } = await companiesAPI.create({ data });
      ...
      throw new Error(responseData ? JSON.stringify(responseData) : "No se pudo registrar la empresa.");
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "No se pudo registrar la empresa.");
    }
  };
