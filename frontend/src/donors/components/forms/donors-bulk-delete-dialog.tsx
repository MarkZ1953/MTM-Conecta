...
type DonorsBulkDeleteDialogProps = {
  ids: number[];
  open: boolean;
...
export function DonorsBulkDeleteDialog({
  ids,
  open,
  setOpen,
  onSuccess,
}: DonorsBulkDeleteDialogProps) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = ids.map((id) => donorsAPI.softDelete({ id }));
      ...
      toast.error("Ocurrió un error al eliminar los donantes.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <Dialog
      header="Eliminar Donantes Seleccionados"
      visible={open}
      onHide={closeDialog}
      modal
      draggable={false}
      ...
      <div className="flex align-items-start gap-3">
        ...
        <div>
          ...
          <p className="m-0 text-700">
            ¿Estás seguro de que deseas eliminar permanentemente <strong>{ids.length}</strong> donante(s) seleccionado(s)?
          </p>
        </div>
      </div>
    </Dialog>
