...
type EventActBulkDeleteDialogProps = {
  ids: number[];
  open: boolean;
...
export function EventActBulkDeleteDialog({
  ids,
  open,
  setOpen,
  onSuccess,
}: EventActBulkDeleteDialogProps) {
  ...
  const onConfirm = async () => {
    ...
    try {
      ...
      const deletePromises = ids.map((id) => eventActAPI.softDelete({ id }));
      ...
      toast.error("Ocurrió un error al eliminar las actas.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  return (
    <Dialog
      header="Eliminar Actas Seleccionadas"
      visible={open}
      onHide={closeDialog}
      modal
      draggable={false}
      ...
      <div className="flex align-items-start gap-3">
        ...
        <div>
          ...
          <p className="m-0 text-700">
            ¿Estás seguro de que deseas eliminar permanentemente <strong>{ids.length}</strong> acta(s) seleccionada(s)?
          </p>
        </div>
      </div>
    </Dialog>
