...
export const UsersPage = () => {
  ...
  const withRole = usersList.filter(
    (u) => roleNames(u.groups).length > 0,
  ).length;
  ...
  const columns: ColumnDef<User>[] = [
    {
      ...
      cell: ({ row: { original: u } }) => (
        <div className="rp-person">
          <div className={`rp-avatar rp-av-${Number(u.id) % 6}`}>
            {initials(u)}
          </div>
          <div>
            <div className="rp-person-name">
    ...
    { accessorKey: "email", header: "Correo" },
    {
      ...
      cell: ({ row: { original: u } }) => {
        ...
        if (roles.length === 0)
          return (
            <span className="rp-badge inactive">
              <span className="dot" /> Sin rol
            </span>
          );
        return (
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {roles.map((r) => (
              <span key={r} className="rp-badge upcoming">
                {r}
              </span>
            ))}
          </div>
    ...
    },
    {
      ...
      cell: ({ row: { original: u } }) =>
        u.is_active !== false ? (
          <span className="rp-badge active">
            <span className="dot" /> Activo
          </span>
        ) : (
          <span className="rp-badge inactive">
            <span className="dot" /> Inactivo
          </span>
        ),
    },
    {
      ...
      cell: ({ row: { original: u } }) => (
        <div className="rp-row-actions">
          <button className="rp-act" title="Editar" onClick={() => openEdit(u)}>
  ...
  return (
    <div className="rp">
      <div className="rp-header">
        <div>
          <h1 className="rp-title">
            Usuarios{" "}
            <span className="count">{fmt(totalCount)} registrados</span>
          </h1>
      ...
      <div className="rp-stats">
        ...
        <div className="rp-stat">
          <div className="rp-stat-pill ink">
        ...
        <div className="rp-stat">
          <div className="rp-stat-pill pink">
      ...
      <DataTable
        ...
        selectionActions={
          <button
            className="rp-btn rp-btn-danger-ghost"
